...
if sType=='pm':
    ...
    if forcing==7:
        ...
        if fbmult:
            ...
            ju.logger.info('fbmult: MM: {:4d}, Nxx-MM: {:4d}, xx[{:4d}, {:4d}]: [{}, {}]'.format(MM, Nxx-MM,MM,Nxx-MM,x[MM],x[Nxx-MM]))
            if B is not None:
...
if sType=='gc':
    ...
    if forcing==1:
        ...
        wux3=wux3/wux3.max()
        ju.logger.info('1860 xx[{:4d} {:4d}]: [{:9.5f} {:9.5f}], w [{:9.1e},{:9.1e},{:9.1e}]'.format(MM, Nxx-MM,xx[MM,0,0], xx[Nxx-MM,0,0],wux3[MM-2,0,0],wux3[MM-1,0,0],wux3[MM,0,0]))
        if db:
        ...
        if fbmult:
            ju.logger.info('MM: {}, Nxx: {}, xx[-1]: {}'.format(MM,Nxx,xx[-1]))
            bmultls = slice(0, MM)
            bmultl  = wux3[bmultls]
            ...
            wux8=wux8/wux8.max()
            MM=np.int64(np.ceil(M*math.pi/maxkx/dxx))
            ju.logger.info('MM: {}, Nxx: {}'.format(MM,Nxx))
            bmultrs = slice(Nxx-MM, Nxx)
            ...
            if PIDX is not None:
                if PIDX >= xx[Nxx-MM]:
                    ju.logger.info('PIDX {:6.3f} is greater than forcing point xx[{}]: {}, xx[{}]:{}, Nxx:{},'.format(PIDX, Nxx-MM,xx[Nxx-MM],MM,xx[MM],Nxx))
                    #ju.set_error('PIDX is greater than forcing point')
            ju.check_error()
        ...
        if not ddiv:
            ...
            if gclf:
                if ddiv:
                    if ju.mpirank==0:
                        ...
                        rfuI=rfu.integrate('x')['g']*rfu['g']
                        #ju.logger.info('type(rful): {}, rful.shape {}'.format(dim,szxx,ju.jtype(rfu),rfu['g'].shape,rfuI.shape))
                        if dim==2: rful = rfuI[:,0]
                        else:      rful = rfuI[:,0,0]
                rful=ju.comm.bcast(rful.reshape(szxx),root=0)    
            ...
            if   gclf and gcrf: rfu['g'] = U*(rfur + rful*state_ev['divz'][zzslices].reshape(szzzl)**2)
            elif gclf:          rfu['g'] = U*(       rful*state_ev['divz'][zzslices].reshape(szzzl)**2)
...
while solver.ok and not ju.abort:
    ...
    if sType=='pm' and forcing==7:
        ...
        if topd: # dynamic evolution of top divergence
            ...
            w.set_scales(AA)
            if topdfield:
                #ju.srange('avar     topu range: [{:5.3f} {:5.3f}]',avar['topu'])
                #ju.srange('avar     topv range: [{:5.3f} {:5.3f}]',avar['topv'])
                #ju.srange('avar     topw range: [{:5.3f} {:5.3f}]',avar['topw'])
                #ju.srange('avar     topd range: [{:5.3f} {:5.3f}]',avar['topd'])
                #ju.srange('state_ev topd range: [{:5.3f} {:5.3f}]',state_ev['topd'])
    ...
    if sType=='gc' and np.isfinite(X): X0=copy.copy(X)
