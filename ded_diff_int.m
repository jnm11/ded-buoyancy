function [a b]=ded_diff_int(a,p,parity,dx,calc_diff,calc_int,calc_NS,fd)
...
if ~isempty(fd)
  c.fd=ichebf2c(fd,dimz);
  if ~isfield(c,'bd') & isfield(c,'b'); c.bd=c.b.*c.fd; end
  if ~isfield(c,'sd') & isfield(c,'s'); c.sd=c.s.*c.fd; end
  if ~isfield(c,'ud') & isfield(c,'u'); c.ud=c.u.*c.fd; end
  if ~isfield(c,'vd') & isfield(c,'v'); c.vd=c.v.*c.fd; end
  if ~isfield(c,'wd') & isfield(c,'w'); c.wd=c.w.*c.fd; end
end
...
if calc_diff
  ...
  for j=1:length(nmd)
    ...
    f=['d' e 'dz'];
    if ~isfield(c,f)
      c.(f)=ichebdiffc(c.(e),dimz,p.H,1,1);
    end
  end
  ...
  for j=1:length(nmdd)
    ...
    f=['d' e 'dzz'];
    if ~isfield(c,f)
      c.(f)=ichebdiffc(c.(e),dimz,p.H,2,1);
    end
  end
end
if calc_diff
  ...
  if ~isfield(c,'udwdz')&isfield(c,'ududx')&isfield(c,'ud'); disp('Calculating udwdz'); c.udwdz = c.ud - c.ududx;  end
  ...
  if ~isfield(c,'wdudx')&isfield(c,'wdwdz')&isfield(c,'wd'); disp('Calculating wdudx'); c.wdudx = c.wd -c.wdwdz;  end
  
  if ~isfield(c,'wdudz')&isfield(c,'duwdz')&isfield(c,'wdudz')&isfield(c,'udwdz'); disp('Calculating wdudz'); c.wdudz  =  c.duwdz - c.udwdz;end
  if ~isfield(c,'udwdx')&isfield(c,'duwdx')&isfield(c,'duwdx')&isfield(c,'wdudx'); disp('Calculating uduwx'); c.udwdx  =  c.duwdx - c.wdudx;end
...
cfn=fieldnames(c);
c.z=linspace(0,p.H,2*max(sz(:,dimz)))';
for j=1:length(cfn)
  f=cfn{j};
