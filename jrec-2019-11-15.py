...
class jit:
    ...
    def clear(self):
        if self.avrg:
            for a in self.nm: self.var[a]=np.zeros(self.lsize)
...
class javrg:
    def __init__(self,a,p,reset):
        ...
        if ju.mpirank==0:
            pv=set(a.pv)
            self.v1a=list(set(
                ('rfu','rfv','rfw','d','p','s','b','u','v','w','SR','OM',
    ...
    def update(self, dt, v):
        ...
        for a in self.ji: 
            if a.avrg:
                ...
                if 'ru' in self.vr:            a.var['ru']        += dt*a.var['u']*np.sqrt(dt/a.var['uu'])
                if 'rb' in self.vr:            a.var['rb']        += dt*a.var['b']*np.sqrt(dt/a.var['bb'])
                if 'rd' in self.vr:            a.var['rs']        += dt*a.var['b']*np.sqrt(dt/a.var['ss'])
                a.n  += 1
                a.dt += dt
    ...
    def update(self, dt):
        ...
        if self.dtjadv:
            if self.dim==2:
                ...
                v['ududz']=self.reduce(v['u']*der['z']['u'],self.parity['ududz'])
                v['udwdx']=self.reduce(v['u']*der['x']['w'],self.parity['udwdx'])
                v['udwdz']=self.reduce(v['u']*der['z']['w'],self.parity['udwdz'])
                v['wdudx']=self.reduce(v['w']*der['x']['u'],self.parity['wdudx'])
                v['wdudz']=self.reduce(v['w']*der['z']['u'],self.parity['wdudz'])
                v['wdwdx']=self.reduce(v['w']*der['x']['w'],self.parity['wdwdx'])
        ...
        if self.dtjsw:
            if self.dim==3:   #Strain rate tensor squared
                v['SR']   = self.square(der['x']['u'])+self.square(der['y']['v'])+self.square(der['z']['w']) + (self.square(der['x']['v']+der['y']['u'])+self.square(der['x']['w']+der['z']['u'])+self.square(der['y']['w']+der['z']['v']))/2
