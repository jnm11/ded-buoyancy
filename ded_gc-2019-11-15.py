...
Options:
  ...
  -N, --Noise=<>        : amplitude for noise forcing
      ...
      --Scd=<>          : Schmidt number for dynamic divergence 
      ...
      --ddiv=<>         : Dynamic divergence evolution
      --ddivq=<>        : Dynamic divergence evolution using integrated radial velocity
      --ddddiv=<>       : Diffuse domain dynamic divergence evolution
      --divxp=<>        : Injection divergence must be positive
      --divxI=<>        : Scale divx so that u is zero
      --wdivxl=<>       : Scale factor for left wdivx
      --wdivxr=<>       : Scale factor for right wdivx
      --ddivscl=<>      : Dynamic divergence scale
      --db=<>           : dynamic buoyancy
...
ddddiv   = dp.get_bool_param(param,'ddddiv')
ddiv     = dp.get_bool_param(param,'ddiv')
ddivq    = dp.get_bool_param(param,'ddivq')
ddivdf   = dp.get_bool_param(param,'ddivdf')
ddivscl  = dp.get_bool_param(param,'ddivscl')
...
if sType=='pm':
    ...
    if ddiv:
        ...
        dAAnwdr=dAA*nwdr
        wdrn = 1-Hrr
        if ddivq: # Flux dynamic divergence integrate vr over each horizontal region
    ...
    if Scd is not None: # Schmidt number for diffusion of dynamic divergence
        Dc    = np.zeros((Nxx+1,),dtype=np.float64)
        Ddivx = np.zeros((Nxx+1,),dtype=np.float64)
    ...
    if topq or topdivr or ddddiv: # calculate x and y components of r unit vector making it go to zero at the edges
    ...
    if ddddiv: # Diffuse domain dynamic divergence
        ddf  = domain.new_field()
        problem.parameters['ddfy'] = domain.new_field()
        problem.parameters['ddfz'] = domain.new_field()
        ju.set_parity(ddf, Tx,Ty,Tz, 1, 1, 1)
        ju.set_parity(problem.parameters['ddfy'],Tx,Ty,Tz,0,-1, 1)
        ju.set_parity(problem.parameters['ddfz'],Tx,Ty,Tz,0, 1,-1)
        ddf['g'] = dftf.heaviside2(math.pi*y/HW,math.pi*z/HH,Ny)*(1-FFTAx.Ndf(L -x,1))*(1-FFTAx.Ndf(   x,4))
        dftf.sawtooth(problem.parameters['ddfy'],domain,'y')
        dftf.sawtooth(problem.parameters['ddfz'],domain,'z')
        problem.parameters['ddfy']['g'] *= ddf['g']/np.maximum(1e-10,r)
        problem.parameters['ddfz']['g'] *= ddf['g']/np.maximum(1e-10,r)
    ...
    if forcing>6: # Plume
        ...
        #if pwul: ful =  wul*wdl       # force necessary to accelerate fluid source
        #if pwur: fur = -wur*wdr       # force necessary to decelerate  fluid sink
...
if sType=='gc':
    ...
    if xb is not None:
        ...
        ifb.set_scales(AA)
        if xa is not None: ifb['g'] = hbzz*(FFTAx.heaviside(xx-xa,1)[0]-FFTAx.heaviside(xx-xb,1)[0])
        else:              ifb['g'] = hbzz* FFTAx.heaviside(xb-xx,1)[0]
    if Tz=='Cheb':
        if param['lbc']=='slip':
    ...
    if forcing==1:
        ...
        (wdl,wdls) = FFTAx.dfsl(xx,2)             # Forcing for left divergence
        (wdr,wdrs) = FFTAx.dfsl(xx-L,2)           # Forcing for right divergence
        (sdl,sdls) = FFTAx.dfsl(xx,2)             # Sampling region for left divergence
        wul        = FFTAx.f(   xx,2)[wdls,...]   # Forcing for left velocity
        wur        = FFTAx.f(   xx-L,2)[wdrs,...] # Forcing for left divergence
        ful=wul*wdl
        fur=wur*wdr
        ...
        if not pwul: ful=None
        if not pwur: fur=None
        
        if x4>0:
            if fbmult: (wbl,wbls)=FFTAx.heavisidel(xx-x4,1)  # Fix density profile left
            else:      (wbl,wbls)=FFTAx.dfslw(  xx,x4)  # Fix density profile left
            
            wbl=wbl/wbl.max()    
        if x5>0:
            if fbmult:
                (wbr,wbrs)=FFTAx.heavisider(xx+x5-L,1)  # Fix density profile right
            else:
                (wbr,wbrs)=FFTAx.dfslw(L-xx,x5)  # Fix density profile right
                wbm = FFTAx.heavisider(xx-L/2,4)[0]
                wbm = wbm/wbm.max()
            wbr=wbr/wbr.max()
        
        if x6>0:
            #(sbl,sbls)=FFTAx.dfslw(  xx,x6)  
            (sbl,sbls)=FFTAx.heavisidel(xx-x6,1)  # Sample density profile left
            sbl=sbl.reshape((sbl.shape[0],1))/(dxx*sbl.sum())
            
        if ddiv:
            #wleft = gyz.new_field(Py=1,Pz=1)
        ...
        if db:
            ...
            if PIDX is not None:
                if PIDX >= L-x6: ju.logger.info('PIDX {:6.3f} is greater than forcing point {}'.format(PIDX, L-x6))
            ...
            if fbmax: bmaxf = wux3*hbzz
                ...
                nwb['g'] += wux3
                if fbmin: nfb['g'] = hbzz * FFTAx.heaviside(L/2-xx,2)[0]
                else:     nfb['g'] = hbzz * wux3
                if fbmin:  bmin  = FFTAx.heaviside(L-0.5-xx,2)[0]
                else:  nwb['g'] += np.flip(wux3)


        if pwul or pwur or pwvl or pwvr or pwwl or pwwr:
            ...
            if ddiv:
                ...
                #ju.logger.info('wdl {} wdr {} wul {} wur {} sdl {}'.format(wdl.shape,wdr.shape,wul.shape,wur.shape,sdl.shape))
                #ju.logger.info('wbl {} wbr {} sbl {}'.format(wbl.shape,wbr.shape,sbl.shape))
                nfd['g'][wdls,...] = wdl*state_ev['divz'][zzslices].reshape(szzzl)
                nfd['g'][wdrs,...] = wdr*U
            else:
                nfd['g'][wdls,...] = wdl*fuzz
                nfd['g'][wdrs,...] = wdr

            if pwul: rfu['g'][wdls,...] = ful*state_ev['divz'][zzslices].reshape(szzzl)**2
            if pwur: rfu['g'][wdrs,...] = fur*U**2
...
ju.write_hdf5_vars('force/fx.hdf5',('dudxrn','pmif','Bwxx','wmult','wbl','wbr','wbm','sbl','sbr','wdl','wdls','sdl','sdls','sdr','sdrs','wdr','wdrs','wul','wur','wux1','wux2','wux3','wux4','wux5','wux6','wux7','wux8','Iux1','Iux2','Iux3','Iux4','Iux5','sminf','bminf','wwl','wwr','ful','fur','wdivx','wuli','wuri'),locals())
...
solver.stop_wall_time = ju.get_sec(param['WallTime'])
...
while solver.ok and not ju.abort:
        ...
        if db:
            ...
            if Ny > 1:
                ...
                if yycolor:
                    ...
                    #print('yycolor {} sbl.shape {} b.shape {} bp.shape {}'.format(yycolor,sbl.shape,b['g'].shape,bp.shape))
                #bp=ju.comm.bcast(bp,0)
            else: bp = dxx*np.sum(sbl*b['g'][sbls,...],axis=(0,))
            #print('rank {} yyrank {} bp {} zzcounts {}'.format(ju.mpirank,yycomm.Get_rank(),bp,zzcounts))
            #ju.logger.error('rank {}, yycolor {}, bp.shape {}'.format(ju.mpirank,yycolor,bp.shape))
            if yycolor:
                #ju.logger.error('rank {}, bp.shape {} zzcounts {}'.format(ju.mpirank,bp.shape,zzcounts[ju.mpirank]))
                #ju.logger.info('rankk {}, gbp.shape {} zzcounts {}'.format(ju.mpirank,bp.shape,zzcounts))
                #print('bp {}'.format(bp))
            ...
            if ju.mpirank==0:
                ...
                state_ev['db']=(dT*state_ev['db']+dt*gbp)/(dT+dt)
                state_ev['db']=state_ev['db']-state_ev['db'].min()
                Ibp=(gIzz*state_ev['db']).sum()/hb
                if Ibp!=0: state_ev['db']=ju.clip01(state_ev['db']/Ibp).reshape((Nzz,))
                if not np.all(np.isfinite(state_ev['db'])):
                    ju.logger.info('bp not finite')
                    ju.logger.info('Ibp/hb {:6.3f}'.format((gIzz*state_ev['db']).sum()/hb))
            ...
            if fbmult: b['g'][wbls,...] += (bps - b['g'][wbls,...])*wbl*bps
            elif fbmax: bmaxf[wbls,...] = bps*wbl
            else:
                problem.parameters['wb']['g'][wbls,...] = bps*wbl
                if wbr is not None: problem.parameters['wb']['g'][wbrs,...] = wbr
                if wbm is not None: problem.parameters['fb']['g'][wbls,...] = bps*wbm
                else: problem.parameters['fb']['g'][wbls,...] = bps
        else:
            if fbmult:
                b.set_scales(AA)
                b['g'][wbls,...] += (hbzz - b['g'][wbls,...])*wbl*hbzz
                 
        if ddiv:
            w.set_scales(AA)
            #gyz.intx(w,sdl,wleft,sdls)
            #wleft.differentiate('z')
            WA=JY.intnr(w.differentiate('z')['g'])
            if yycolor:
                dw  = -dxx*np.sum(sdl*WA[sdls,...],axis=(0,))
            ...
            if ju.mpirank==0:
                ...
                state_ev['divz'] += (dt/dT)*gdw
                Idivz=(gIzz*state_ev['divz']).sum()
                if Idivz!=0: state_ev['divz'] =-state_ev['divz']*H*U/Idivz
            ju.check_error()
            state_ev['divz'] = ju.comm.bcast(state_ev['divz'],0)
            fd['g'][wdls,...]           = wdl*state_ev['divz'][zzslices].reshape(szzzl)
            fd['g'][wdrs,...]           = wdr*U                                           
            #if pwul: rfu['g'][wdls,...] = ful*wrr**2*U**2
            #if pwur: rfu['g'][wdrs,...] = fur*state_ev['divz'][zzslices].reshape(szzzl)**2
            if pwul: fu['g'][wdls,...]   = wul*U
            if pwur: fu['g'][wdrs,...]   = wur*state_ev['divz'][zzslices].reshape(szzzl) 
    ...
    if 'pmss' in param: # moving source for the plume
        ...
        if not ddiv and forcing<7: fu['g'] = wrr*wux4+mu*(1-wux4)*inwdr 
    ...
    if sType=='pm':
        ...
        if ddiv:
            fd.set_scales(AA)
            if ju.mpirank==0:
                if ddivq:   state_ev['divx']['g'] *= np.exp(-dt/dT*avar['Iu']/(L*U))
                if ddivq:   state_ev['divx']['g']  = (dT*state_ev['divx']['g']-dt*avar['q']['g'])/(dT+dt)
                ...
                if Scd is not None: # Diffuse according to cubic rule
                    #state_ev['divx']['g'][ :2]=0 # divx should be even and zero at the ends
                    #state_ev['divx']['g'][-2:]=0
                    Ddivx[1:-1] = np.diff(state_ev['divx']['g']) # Take derivative
                    Dc=Ddivx**2
                    Dcmax=Dc.max()
                    if  Dcmax>0:
                        Dc[1:-1] = (Dc[:-2]+2*Dc[1:-1]+Dc[2:])/4
                        Dc=Dc*min(1/2,dt/(Scd*Re*dxx**2))/Dc.max()
                        state_ev['divx']['g']  += np.diff(Dc*Ddivx) # Perform nonlinear diffusion
                if divxp: state_ev['divx']['g'] = np.maximum(0,state_ev['divx']['g']);
    ...
    if iteration_one:
        for a in ('rfu','rfv','rfw','fu','fv','fw'):
            if a in problem.parameters: JA.savexyz('force/'+a+'h.df5',a, problem.parameters[a]['g'],AA)
...
#yyzzstart  = np.array(ju.comm.gather(list(domain.dist.grid_layout.start(scales=AA)[1:]),root=0))
    ...
    #ju.logger.info('yyzzcounts: {} {}'.format(yyzzcounts.shape,yyzzcounts))
                ...
                #if pmIu: state_ev['divx'] = np.maximum(0,wdivx*(state_ev['divx']-wux4*(dt/dT*Idudx+Anwdr*avar['Iu']*10*dt)))
