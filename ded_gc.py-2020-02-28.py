...
if sType=='gc':
    ...
    if forcing==1:
        ...
        if x4 is not None:
            if   fbmult: (wbl,wbls)=FFTAx.heavisidel(xx-x4,2)  # Fix density profile left
            elif fbmax:  (wbl,wbls)=FFTAx.dfslw(  xx,x4)      # Fix density profile left
            else:
                nwb.set_scales(AA)
                nwb['g'] = FFTAx.heaviside(x4-xx,2)[0]
            if wbl is not None:  wbl=wbl/wbl.max()
            
        if x5 is not None:
            if fbmult:
                (wbr,wbrs)=FFTAx.heavisider(xx+x5-L,2)  # Fix density profile right
            elif fbmax:
                (wbr,wbrs)=FFTAx.dfslw(L-xx,x5)  # Fix density profile right
            else:
                nwb.set_scales(AA)
                nwb['g'] += FFTAx.heaviside(xx+x5-L,2)[0]
            if wbr is not None: wbr=wbr/wbr.max()
        ...
        if db:
            ...
            if not fbmax:
                if nwb is not None: nwb.set_scales(AA)
                if nfb is not None: nfb.set_scales(AA)
            if not fbmin:
                if nwb is not None: nwb.set_scales(AA)

            if fbmax: bmaxf = wux3*hbzz
            if fbmult:
                if nwb is not None: nwb['g'] += wux3
                if nfb is not None and hbzz is not None:
                    if fbmin: nfb['g'] = hbzz * FFTAx.heaviside(L/2-xx,2)[0]
                    else:     nfb['g'] = hbzz * wux3
                if fbmin:  bmin  = FFTAx.heaviside(L-0.5-xx,2)[0]
                else:
                    if nwb is not None: nwb['g'] += np.flip(wux3)

            if not fbmult and not fbmax:
                nfb.set_scales(AA)
...
ju.write_hdf5_vars('force/fx.hdf5',('dudxrn','pmif','Bwxx','wmult','wbl','wbr','sbl','sbr','wdl','wdls','sdl','sdls','sdr','sdrs','wdr','wdrs','wul','wur','wux1','wux2','wux3','wux4','wux5','wux6','wux7','wux8','Iux1','Iux2','Iux3','Iux4','Iux5','sminf','bminf','wwl','wwr','ful','fur','wdivx','wuli','wuri'),locals())
...
while solver.ok and not ju.abort:
    ...
    if forcing==7 and sType=='gc':
        if db:
            ...
            else: problem.parameters['fb']['g'] = bps*FFTAx.heaviside(L/2-xx,2)[0]
        ...
        if ddiv:
            ...
            if ju.mpirank==0:
                ...
                #ju.logger.info('{:7.4f} {:7.4f}'.format(Idivz/H,-U-Idivz/H))
                if False:
                    Idivz=(gIzz*state_ev['divz']).sum()
                    state_ev['divz'] += -U-Idivz/H
                if True:
                    U2=H/(H-hu)*U
                    # u=-U2+a*d
                    # -H*U_2 + a*I = -H U
                    ...
                    state_ev['divz'] = state_ev['divz']-min(state_ev['divz'])
                    Idivz=(gIzz*state_ev['divz']).sum()
                    if Idivz!=0: state_ev['divz'] = state_ev['divz']*H*(U2-U)/Idivz
                    state_ev['divz'] -= U2
                    Idivz=(gIzz*state_ev['divz']).sum()
                    #ju.logger.info('{:7.4f} {:7.4f}  {:7.4f}'.format(Idivz/H,max(state_ev['divz']),min(state_ev['divz'])))
